...
// MARK: SteamNetworkingFakeUDPPort

// This is a weird thing, it's partially a data type and partially an
// 'interface' - but doesn't behave like any of the other interfaces.
//
// We could get the generator to create the methods, would need tweaks
// to understand the odd lifetime stuff.  Leave it for now, return if
// more things like this appear.

public final class SteamNetworkingFakeUDPPort {
    typealias PortPtr = UnsafeMutablePointer<ISteamNetworkingFakeUDPPort>
    private var port: PortPtr
    init(_ port: PortPtr) {
        self.port = port
    }

    deinit {
        SteamAPI_ISteamNetworkingFakeUDPPort_DestroyFakeUDPPort(port)
    }

    /// Steamworks `ISteamNetworkingFakeUDPPort::SendMessageToFakeIP()`
    public func sendMessageToFakeIP(remoteAddress: SteamNetworkingIPAddr,
                                    data: UnsafeRawPointer, count: Int,
                                    sendFlags: SteamNetworkingSendFlags) -> Result {
        var tmp_remoteAddress = CSteamworks.SteamNetworkingIPAddr(remoteAddress)
        return Result(SteamAPI_ISteamNetworkingFakeUDPPort_SendMessageToFakeIP(port, &tmp_remoteAddress, data, UInt32(count), Int32(sendFlags)))
    }
    ...
    public func receiveMessages(outMessages: inout [SteamNetworkingMessage], maxMessages: Int) -> Int {
        ...
        let rc = Int(SteamAPI_ISteamNetworkingFakeUDPPort_ReceiveMessages(port, tmp_outMessages.baseAddress, Int32(maxMessages)))
        outMessages = tmp_outMessages[0..<rc].map { SteamNetworkingMessage($0) }
        return rc
    }

    /// Steamworks `ISteamNetworkingFakeUDPPort::ScheduleCleanup()`
    public func scheduleCleanup(remoteAddress: SteamNetworkingIPAddr) {
        var tmp_remoteAddress = CSteamworks.SteamNetworkingIPAddr(remoteAddress)
        SteamAPI_ISteamNetworkingFakeUDPPort_ScheduleCleanup(port, &tmp_remoteAddress)
    }
}
